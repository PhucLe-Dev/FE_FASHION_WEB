...
import { IProduct, IVariant } from "@/app/interface/ISanPham";
...
export default function Detail({ initialProduct }: DetailProps) {
  ...
  const [sp, setSp] = useState<IProduct | null>(initialProduct);
      ...
      <main className="grid min-h-full place-items-center bg-white px-6 py-24 sm:py-32 lg:px-8">
        <div className="text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-x-6">
            <Link
              href="/"
              className="rounded-md bg-[#c19a6b] px-3.5 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-[#b39168b6] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#c79a63cc]"
            >
              Quay về trang chủ
            </Link>
    ...
    <div>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
                  ...
                  {images.slice(1).map((thumbnail, index) => (
                    ...
                    >
                      <img
                        ...
                        style={{
                          borderColor: images[0] === thumbnail ? "#C19A6B" : "transparent",
                        }}
                      />
                    </div>
                  ))}
            ...
            <div className="lg:w-1/2">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <button className="flex items-center justify-center cursor-pointer border-2 border-[#C19A6B] px-8 py-4 hover:bg-[#C19A6B] hover:text-white transition tracking-wider font-medium uppercase flex-1">
              ...
              <div className="mb-8">
                <div className="flex justify-between border-b border-gray-200">
                  <button
                    ...
                    className={`px-4 py-3 font-medium text-primary ${activeTab === "description" ? "border-b-2 border-primary" : ""}`}
                    onClick={() => handleTabClick("description")}
                  >
                    Mô tả sản phẩm
                  </button>
                  <button
                    className={`px-4 py-3 font-medium text-primary ${activeTab === "details" ? "border-b-2 border-primary" : ""}`}
      ...
      <div className="container mx-auto px-6">
        {sp.tags.map((tag, index) => (
          <Link href={`/tag/${tag}`} key={index} className="text-base text-[#c19a6b] hover:text-primary mr-2 hover:underline">
            #{tag}
          </Link>
        ))}
      </div>
