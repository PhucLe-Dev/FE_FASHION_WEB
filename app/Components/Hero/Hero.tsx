  ...
  return (
    <section className="relative h-screen flex items-center pt-16 overflow-hidden">
      ...
      <div className="container mx-auto px-6 z-10">
        <div className="max-w-2xl text-center mx-auto">
          <h1 className="text-5xl md:text-6xl font-playfair font-bold text-white mb-6">
            Timeless <span className="text-[#C19A6B]">Luxury</span>
          </h1>
          <p className="text-white text-xl mb-8 font-light tracking-wider">
            CURATED COLLECTIONS OF THE FINEST DESIGNER PIECES
          </p>
          <button
            className="cursor-pointer bg-[#C19A6B] text-white px-10 py-4 hover:opacity-90 transition tracking-wider font-medium uppercase"
          >
            Mua ngay
          </button>
        </div>
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${
              index === currentSlide ? 'bg-[#C19A6B]' : 'bg-white bg-opacity-50'
            }`}
          />
        ))}
      </div>
      ...
      <button
        ...
        className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 p-2 bg-[#C19A6B] bg-opacity-50 hover:bg-opacity-75 rounded-full transition"
      >
        <CiCircleChevLeft className="w-6 h-6 text-white" />
      </button>
      <button
        onClick={goToNextSlide}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 p-2 bg-[#C19A6B] bg-opacity-50 hover:bg-opacity-75 rounded-full transition"
      >
